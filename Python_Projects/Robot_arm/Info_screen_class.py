...
class InfoScreen:
    ...
    def get_center_of_mass(robot: robot_arm_class.RobotArm):
        center_x = (robot.first_joint_tip[0] + robot.second_joint_tip[0] + robot.third_joint_tip[0]) / 3
        center_y = (robot.first_joint_tip[1] + robot.second_joint_tip[1] + robot.third_joint_tip[1]) / 3
        return int(center_x + robot.base_coord[0]), int(robot.base_coord[1] - center_y)
    ...
    def print_info(self, robot: robot_arm_class.RobotArm):
        ...
        self.info_list['tip coordinates : '] = "({})".format(', '.join(map(str, map(int, tip))))
