#include "ft_ssl.h"

int cst[64] = {
   0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
   0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
   0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
   0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
   0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
   0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
   0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
   0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
};

t_bytes *format_sha256_output(uint32_t *res)
{
    t_bytes *output;
    
    if (!(output = malloc(sizeof(t_bytes))))
        return NULL;
    if (!(output->bytes = malloc(32)))
    {
        free(output);
        return NULL;
    }
    output->nb_bytes = 32;
    ft_memcpy(output->bytes, res, 32);
    return output;
}

t_bytes *ft_sha256(t_bytes data)
{
    unsigned int res[8] = {0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19};
    char* padded_msg;
    unsigned int tmp[8];
    uint64_t nb_blocks = 0;
    unsigned int words[64];
    uint64_t block_index = 0;

    nb_blocks = ceil((data.nb_bytes + 1) / (float)64) + ((data.nb_bytes % 64) >= 56 ? 1 : 0);
    if (!(padded_msg = pad_msg(data)))
        return NULL;
    ((uint64_t*)(padded_msg))[(nb_blocks - 1) * 8 + 7] = __bswap_64(((uint64_t*)(padded_msg))[(nb_blocks - 1) * 8 + 7]);
    for (int i = 0; i < nb_blocks; i++)
    {
        ft_memset(words, 0, 256);
        ft_memcpy(words, &(((uint32_t*)(padded_msg))[i * 16]), 64);
        for (int f = 0; f < 16; f++)
            words[f] = __bswap_32(words[f]);
        for (int l = 16; l < 64; l++)
        {
            words[l] = words[l - 16]
                + (rotr32(words[l - 15], 7) ^ rotr32(words[l - 15], 18) ^ (words[l - 15] >> 3))
                + words[l - 7]
                + (rotr32(words[l - 2], 17) ^ rotr32(words[l - 2], 19) ^ (words[l - 2] >> 10));
        }
        for (int k = 0; k < 8; k++)
            tmp[k] = res[k];
        for (int j = 0; j < 64; j++)
        {
            unsigned int S1;
            unsigned int ch;
            unsigned int temp1;
            unsigned int S0;
            unsigned int maj;
            unsigned int temp2;

            S1 = rotr32(tmp[4], 6) ^ rotr32(tmp[4], 11) ^ rotr32(tmp[4], 25);
            ch = (tmp[4] & tmp[5]) ^ ((~tmp[4]) & tmp[6]);
            temp1 = tmp[7] + S1 + ch + cst[j] + words[j];
            S0 = rotr32(tmp[0], 2) ^ rotr32(tmp[0], 13) ^ rotr32(tmp[0], 22);
            maj = (tmp[0] & tmp[1]) ^ (tmp[0] & tmp[2]) ^ (tmp[1] & tmp[2]);
            temp2 = S0 + maj;

            tmp[7] = tmp[6];
            tmp[6] = tmp[5];
            tmp[5] = tmp[4];
            tmp[4] = tmp[3] + temp1;
            tmp[3] = tmp[2];
            tmp[2] = tmp[1];
            tmp[1] = tmp[0];
            tmp[0] = temp1 + temp2;
        }
        for (int k = 0; k < 8; k++)
            res[k] += tmp[k];
    }
    free(padded_msg);
    for (int k = 0; k < 8; k++)
        res[k] = __bswap_32(res[k]);
    return format_sha256_output(res);
}
