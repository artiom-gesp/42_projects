...
void	print_L_float(va_list *ap, t_plist *list)
{
	long double	save;
	long long	fpart;
	char		*part1;
	...
	save = (long double)va_arg(*ap, long double);
	fpart = save;
...
void	print_nfloat(va_list *ap, t_plist *list)
{
	double		save;
	long long	fpart;
	char		*part1;
...
{
	if (list->flag == 5)
		print_L_float(ap, list);
	else
		print_nfloat(ap, list);
}
